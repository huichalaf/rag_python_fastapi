...
def get_daily_spend(user):
    df = pd.read_csv(f"costs/costs_{date.today()}_{user}.csv")
    return float(df.price.sum())

def get_total_spend(user):
    files2 = os.listdir(f"costs/")
    files = []
    for file in files2:
        if user in file:
            files.append(file)
    ...
    total_price = 0
    for file in files:
        df = pd.concat([df, pd.read_csv(f"costs/{file}")], ignore_index=True)
